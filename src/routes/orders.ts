...
orderRouter.post(
  "/",
  body("id_client").optional().isInt().withMessage("ID de cliente inválido"),
  handleInputErrors,
  createOrder
);
...
orderRouter.get(
  "/:id",
  param("id").isInt().withMessage("ID de pedido no válido"),
  handleInputErrors,
  getOrderById
);
...
orderRouter.post(
  ...
  body("quantity").optional().isInt({ min: 1 }).withMessage("Cantidad inválida"),
  handleInputErrors,
  addOrderItem
);
...
orderRouter.patch(
  "/items/:id_item",
  param("id_item").isInt().withMessage("ID de item no válido"),
  body("quantity").optional().isInt({ min: 1 }).withMessage("Cantidad inválida"),
...
orderRouter.post(
  ...
  body("amount_cents").notEmpty().isInt({ min: 0 }).withMessage("Monto inválido"),
...
----------------------------------------------------- */
orderRouter.delete(
  "/:id/items/:id_item/ingredients/:id_ingredient",
  param("id").isInt(),
  param("id_item").isInt(),
  param("id_ingredient").isInt(),
  handleInputErrors,
  removeOrderItemIngredient
);
